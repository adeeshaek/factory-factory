...
flowchart LR
    ...
    ChatWS --> QueueH["queue-message.handler.ts:44<br/>buildQueuedMessage (utils.ts:17)<br/>persists a QueuedMessage<br/>(queued.ts:17) from the original text"]
    QueueH -->|"QueuedMessage sits in<br/>SessionStore.queue"| Dispatch["tryDispatchNextMessage<br/>chat-message-handlers.service.ts:205"]
    Dispatch --> DispatchMsg["dispatchMessage (612)<br/>buildMessageContent (759)<br/>reads the dequeued QueuedMessage"]
    DispatchMsg --> AttachProc["processAttachmentsAndBuildContent<br/>attachment-processing.ts:271<br/>merges msg.text + any attachments<br/>into a string or AgentContentItem[]"]
    AttachProc --> SendSession["session.service.ts:85<br/>sendSessionMessage<br/>accepts a string or a content-item array"]
